...
5. In Google Cloud Console, add the exact callback URL shown by `GET /oauth/debug` to **Authorized redirect URIs**.
   - ✅ Correct examples for local dev: `http://localhost:5000/oauth2callback` and `http://127.0.0.1:5000/oauth2callback`
   - ❌ Not enough: `http://localhost:5000` (missing `/oauth2callback`)
